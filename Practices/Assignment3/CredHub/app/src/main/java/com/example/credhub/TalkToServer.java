...
public class TalkToServer extends AsyncTask<String, String, String> {
    ...
    /**
     * Perform the HTTPS connection to the server adding the authentication information in the header
     */
    ...
    private void postImport() {
        ...
        if (activity != null) {
            ...
            UserDBHelper helper = new UserDBHelper(callerActivity.get().getApplicationContext());

            sprefs = callerActivity.get().getSharedPreferences(MyPreferences, Context.MODE_PRIVATE);

            Cursor cursor = helper.dbGetCredential(callerActivity.get().getApplicationContext(), response[0], sprefs);

            if (cursor.moveToFirst()) { // Update

                helper.dbUpdate(response[0], response[1], response[2], callerActivity.get().getApplicationContext(), sprefs);

            } else {

                helper.dbInsert(response[0], response[1], response[2], callerActivity.get().getApplicationContext(), sprefs);
